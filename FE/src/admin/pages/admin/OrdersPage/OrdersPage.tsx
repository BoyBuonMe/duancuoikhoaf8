...
import { getOrders } from "@/admin/features/orders/ordersSlice";
import {
  type Order,
  type OrderStatus,
} from "@/admin/services/orders/ordersService";
...
const STATUS_COLORS: Record<OrderStatus, string> = {
  ...
  awaiting_pickup: "bg-violet-50 text-violet-700 ring-violet-100",
...
function paymentStatusLabel(order: Order) {
  if (order.isPay) return "Đã thanh toán";
  if (order.status === "payment_failed") return "Thanh toán lỗi";
  return "Chưa thanh toán";
}

function paymentStatusClassName(order: Order) {
  if (order.isPay) return "bg-emerald-50 text-emerald-700 ring-emerald-100";
  if (order.status === "payment_failed") return "bg-red-50 text-red-700 ring-red-100";
...
function getOrderStatus(order: Order): OrderStatus {
  ...
  if (order.status === "awaiting_pickup") return "shipping";
                  ...
                  <tr key={getOrderId(order)} className="transition-colors hover:bg-gray-50">
                    ...
                    <td className="whitespace-nowrap px-4 py-3 text-center">
                      <div className="flex flex-col items-center gap-1">
                        ...
                        <span
                          className={`inline-flex whitespace-nowrap rounded-lg px-2.5 py-1 text-xs font-semibold ring-1 ${paymentStatusClassName(order)}`}
                        >
